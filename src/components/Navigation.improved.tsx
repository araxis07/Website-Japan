...
const Navigation = ({ dict, lang }: NavigationProps) => {
  ...
  // Pre-calculate text content to prevent hydration mismatch
  const siteTitle = mounted ? (lang === 'ja' ? '日本旅行' : 'Japan Travel') : '日本旅行'
  const langToggleText = mounted ? (lang === 'ja' ? 'EN' : '日本語') : 'EN'
